...
def heatmap(img, ax=None, cmap='inferno', fontsize=14, precision=3, bgbox = True, vmin=None, vmax=None) :
    if ax is None :
        _,ax=plt.subplots(1)
        
    ax.imshow(img,cmap=cmap,origin='upper', vmin=vmin, vmax=vmax)
    ax.set(xticks=np.arange(0,img.shape[1]),yticks=np.arange(0,img.shape[0]))
    props = dict(boxstyle='round', facecolor='lightgray', alpha=1) 
    midval = np.mean([img.max(),img.min()])
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            if bgbox :
                ...
                text = ax.text(j, i, "{0:0.{1}f}".format(img[i, j],precision),
                               ha="center", va="center", fontsize=fontsize, color = 'k' if midval < img[i,j] else 'w')
